...
def read_in_tiger_corpus() -> List[TokenList]:
    """
...
def export_tiger_corpus():
    ...
    # --> train:
    with open(os.path.join(ROOT_DIR, f"data/conllu_files/tiger_train.conllu"), "w") as f:
        for compiled_sentence in train:
            f.write(compiled_sentence.serialize())
    # --> test:
    with open(os.path.join(ROOT_DIR, f"data/conllu_files/tiger_test.conllu"), "w") as f:
        for compiled_sentence in dev:
            f.write(compiled_sentence.serialize())
    # --> dev:
    with open(os.path.join(ROOT_DIR, f"data/conllu_files/tiger_dev.conllu"), "w") as f:
